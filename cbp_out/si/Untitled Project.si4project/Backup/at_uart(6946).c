...
#include "debug.h"
extern  unsigned int CRC16_XMODEM(unsigned char *puchMsg, unsigned int usDataLen);
struct at_uart {
    u32 baudrate;
    int flowcontrol;
    const char *dev_name;
...
#define UART_DB_SIZE          512
#define UART_BAUD_RATE        1000000

#define UART_DB_TX_PIN        IO_PORTC_04
#define UART_DB_RX_PIN        IO_PORTC_05
...
#endif
extern int ble_at_send_data_default(u8 *data,u8 len);
extern int at_send_audio_data(u8 *data,u16 len);
...
void at_cmd_rx_handler(void)
{
	...
	char buffer[8] = {0xa5,0x5a,0x00,0x04,0x87,0x01,0x00,0x00};
...
/*************************这两条打印调试完一定要关掉*********************************/
	...
	if(*((u8*)__this->pRxBuffer) != AT_PACKET_TYPE_CMD)
	{
		...
		//log_info("IS NOT TYPE_CMD, len(%d)", __this->data_length);
		if(uart_cmd_deal_busy == 0) {
			uart_cmd_deal_busy = 1;
			switch(pRxData[4])
			{
				case 0x0A:
					at_send_audio_data(__this->pRxBuffer, __this->data_length);
					break;
				case 0x07:
					r_printf("YPE_CMD POWER_OFF\r\n");
//					log_info_hexdump(__this->pRxBuffer, __this->data_length);
					crc16 = CRC16_XMODEM(&buffer[2], 4);
					buffer[6] = crc16>>8;
					buffer[7] = crc16;
					at_uart_send_packet(buffer,8);
					 extern void app_set_soft_poweroff(void);
					 	  app_set_soft_poweroff();
					break;
				default:
					frame_len = pRxData[3] + 5;
					if(cur_time - Pre_time >= 1000) {
						ble_at_send_data_default(__this->pRxBuffer, frame_len);
						Pre_time = cur_time;
					}
					if(__this->data_length > frame_len) {
						//at_send_audio_data(&pRxData[frame_len], (__this->data_length-frame_len) );
					}
					break;
			}
			uart_cmd_deal_busy = 0;
		}
