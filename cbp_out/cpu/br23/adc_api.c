...
void update_vbg_vbat_adc_value(void)
{
    u16 old_adc_res;
    static u16 vbg_vbat_flag = 0;
    local_irq_disable();
    old_adc_res = _adc_res;
    if (vbg_vbat_flag == 0) {
        adc_sample(AD_CH_LDOREF);
        while(!(JL_ADC->CON & BIT(7)));
        tmp_vbg_adc_value = JL_ADC->RES;
        /* printf("vbg = %d\n", tmp_vbg_adc_value); */
    } else {
...
void _adc_init(u32 sys_lvd_en)
{
    ...
    vbg_adc_value /= 10;
    tmp_vbg_adc_value = vbg_adc_value;
