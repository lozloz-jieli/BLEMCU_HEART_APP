...
/* } */
 ...
 unsigned int CRC16_XMODEM(unsigned char *puchMsg, unsigned int usDataLen)  
{  
	  int i;
...
void at_send_event(u8 opcode, const u8 *packet, int size)
{
	...
	u32 check_sum = 0;
	int send_cnt = 1;
	u8* pData = (u8*)__this->pAT_buffer;
	switch(opcode)
	{
		case AT_EVT_BLE_DISCONNECTED:
			pData[index++] = 0xA5;
			pData[index++] = 0x5A;
			pData[index++] = 0x00;
			pData[index++] = 0x04;
			...
			check_sum = CRC16_XMODEM(&pData[2], (index-2));
			pData[index++] = (check_sum&0xFF00)>>8;
			pData[index++] = (check_sum&0xFF);
			send_cnt = 3;
			break;
		case AT_EVT_BLE_CONNECTED:
			pData[index++] = 0xA5;
			pData[index++] = 0x5A;
			pData[index++] = 0x00;
			pData[index++] = 0x04;
	...
	if(index != 0) {
		do {
			at_uart_send_packet(pData, index);
		}while(send_cnt--);
	}
#endif
}
