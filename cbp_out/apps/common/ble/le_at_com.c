...
#include "system/audio_enc_api.h"
...
//采样率压缩
static u8 *p_resample_workBuf = NULL;
    ...
    case HCI_EVENT_PACKET:
        ...
        case HCI_EVENT_LE_META:
            ...
            case HCI_SUBEVENT_LE_CONNECTION_COMPLETE:
				...
				//att_set_ccc_config(ATT_CHARACTERISTIC_ae02_01_CLIENT_CONFIGURATION_HANDLE, 1);
                break;
...
static int app_send_user_data(u16 handle, u8 *data, u16 len, u8 handle_type)
{
	...
	}while( (bloop--) && (ret == BLE_BUFFER_FULL) );
...
static int BLE_DELAY_TIMER_ID=0;
void BLE_DELAY_ENABLE()
{
	ble_module_enable(1);	//jimmy add , enable ble delay 2s
     sys_timeout_del(BLE_DELAY_TIMER_ID);
}
...
void bt_ble_init(void)
{
   ...
   // ble_module_enable(1);   //jimmy add , enable ble delay 2s
    BLE_DELAY_TIMER_ID=sys_timeout_add(NULL, BLE_DELAY_ENABLE, 2000);

#if TEST_SEND_DATA_RATE
    server_timer_start();
#endif

	if(NULL == p_resample_workBuf)
	{
		p_resample_workBuf = malloc(ima_code_buf());
		if(p_resample_workBuf){
			ima_code_init(p_resample_workBuf);
		}
	}
}
...
{
	log_info("JIMMY ADD ble_at_send_event\r\n");
...
{
    return app_send_user_data((ATT_CHARACTERISTIC_ae02_01_CLIENT_CONFIGURATION_HANDLE-1), data, len,ATT_OP_NOTIFY);
}
...
static s16 pResamplebuf[128/2]  __attribute__((aligned(4)));	   //re sample memory
static u8 pCompressbuf[64] __attribute__((aligned(4))); 	  //rx memory
static u8 p_temp_inbuff[260]  __attribute__((aligned(4))); 	  //rx memory

static u16 audio_resample(s16* in_buff, s16* out_buff, u16 in_len)
{
	u16 outlen = 0;
	if(NULL == sw_src_api) {
	    ...
	    rs_para_obj.nch = 1;
	    rs_para_obj.new_insample = 8000;
	    rs_para_obj.new_outsample = 2000;
	...
	if(sw_src_api && sw_src_buf) {
		/*这里的运算以点为单位*/
	    outlen = sw_src_api->run(sw_src_buf, in_buff, (in_len/2), out_buff);
        outlen = outlen << 1;
		if(outlen != 64) {
			printf("[RE SAMPLE] Out in_len = %d len = %d\r\n", in_len, outlen);
		}
	}

	//printf("[RE SAMPLE] Out len = %d\r\n", outlen);
	return outlen;
}

#if 0
const short testdata[4000] =
{
0x0000  ,0x12f1  ,0x2528  ,0x35f2  ,0x44a9  ,0x50bc  ,0x59b5  ,0x5f3b  ,0x6119  ,0x5f3b  ,0x59b5  ,0x50bc  ,0x44a9  ,
0x35f2  ,0x2528  ,0x12f1  ,
0x0000  ,0xed0f  ,0xdad8  ,0xca0e  ,0xbb57  ,0xaf44  ,0xa64b  ,0xa0c5  ,0x9ee7  ,0xa0c5  ,0xa64b  ,0xaf44  ,0xbb57  ,
0xca0e  ,0xdad8  ,0xed0f  ,
0x0000  ,0x12f1  ,0x2528  ,0x35f2  ,0x44a9  ,0x50bc  ,0x59b5  ,0x5f3b  ,0x6119  ,0x5f3b  ,0x59b5  ,0x50bc  ,0x44a9  ,
0x35f2  ,0x2528  ,0x12f1  ,
0x0000  ,0xed0f  ,0xdad8  ,0xca0e  ,0xbb57  ,0xaf44  ,0xa64b  ,0xa0c5  ,0x9ee7  ,0xa0c5  ,0xa64b  ,0xaf44  ,0xbb57  ,
0xca0e  ,0xdad8  ,0xed0f  ,
...
s16 test_cur_index = 5000;
s16 test_cur_index2 = 0;

int at_send_audio_data(u8 *data,u16 len)
{
	u16 audio_len = 0;
    short *p_data = (short* )data;

	if((NULL != p_resample_workBuf) && (len > 256) && (len <= 280*4))
	{

		u16 i = 0;

		if((test_cur_index+128) > 4000) {
			//test_cur_index = 0;
			return 0;
		}
		audio_len = audio_resample(&testdata[test_cur_index], pResamplebuf, (len-7));
		test_cur_index += (len-7)/2;
		//printf("index = %d\r\n", test_cur_index);
		//log_info_hexdump(pResamplebuf, audio_len);
		...
		ima_code_run(p_resample_workBuf, pResamplebuf, &pCompressbuf[5], audio_len/2);
		audio_len >>= 2;

		//test_cur_index+=audio_len/2;

		pCompressbuf[0] = 0xA5;
		pCompressbuf[1] = 0x5A;
		pCompressbuf[2] = 0x00;
		pCompressbuf[3] = audio_len + 3;
		pCompressbuf[4] = 0x0A;
		/*
		for(i = 0; i < 20; i++) {
			pCompressbuf[i] = test_cur_index2;
			test_cur_index2++;
		}
		if(test_cur_index2 < 240) {
			ble_at_send_data_default(pCompressbuf, 20);
		}
		*/
		ble_at_send_data_default(pCompressbuf, audio_len+7);
		test_cur_index2 += audio_len+7;
		//printf("[BLE] at_send_audio_data len = %d\r\n", pCompressbuf[3]);
		//log_info_hexdump(pCompressbuf, audio_len+7);
...
int at_send_audio_data(u8 *data,u16 len)
{
	...
	if((NULL != p_resample_workBuf) && (len > 256) \
				&& (len <= 280*4) && (NULL != con_handle) )  {
		memcpy(p_temp_inbuff, p_data, (len-7));
#if 0
		for(i = 0; i < len -7; i++)
		{
			printf("hello");
		...
		audio_len = audio_resample(p_temp_inbuff, pResamplebuf, (len-7));
		//log_info_hexdump(pResamplebuf, audio_len);

		// npoint 为输入样本长度， 输出长度为 npoint >> 1
		ima_code_run(p_resample_workBuf, pResamplebuf, &pCompressbuf[5], audio_len/2);
		audio_len >>= 2;
		pCompressbuf[0] = 0xA5;
		pCompressbuf[1] = 0x5A;
