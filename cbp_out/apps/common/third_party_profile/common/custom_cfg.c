    ...
    for (i = 0 ; i < sizeof(cfg_item_description) / sizeof(cfg_item_description[0]); i++) {
        ...
        if (cfg_item_description[i].real_len != 0) {
            ...
            if (0 == strcmp((const char *)item_name, "ver_info")) {
                custom_cfg_fill(user_ex_cfg, item_data, item_len, CFG_ITEM_VER_INFO, write_flag);
            } else if (0 == strcmp((const char *)item_name, "reset_io")) {
                custom_cfg_fill(user_ex_cfg, item_data, item_len, CFG_ITEM_RESET_IO_INFO, write_flag);
            } else if (0 == strcmp((const char *)item_name, "pilot_lamp_io")) {
                custom_cfg_fill(user_ex_cfg, item_data, item_len, CFG_ITEM_PILOT_LAMP_IO_INFO, write_flag);
            } else if (0 == strcmp((const char *)item_name, "link_key")) {
                custom_cfg_fill(user_ex_cfg, item_data, item_len, CFG_ITEM_LINK_KEY_INFO, write_flag);
            } else if (0 == strcmp((const char *)item_name, "power_io")) {
                custom_cfg_fill(user_ex_cfg, item_data, item_len, CFG_ITEM_POWER_IO_ON_OFF, write_flag);
            } else if (0 == strcmp((const char *)item_name, "ver_info_ext")) {
#if VER_INFO_EXT_COUNT
                fill_authkey_procode(user_ex_cfg, item_data, item_len, write_flag);
#endif
            }
        }
    }
...
static u32 ex_cfg_fill_content(ex_cfg_t *user_ex_cfg, u8 *write_flag)
{
    ...
    custom_cfg_fill(user_ex_cfg, (u8 *)host_name, sizeof(user_ex_cfg->bt_name_cfg.data), CFG_ITEM_BT_NAME, write_flag);
    custom_cfg_fill(user_ex_cfg, cur_edr_address, sizeof(user_ex_cfg->edr_addr_cfg.data), CFG_ITEM_EDR_ADDR, write_flag);
    custom_cfg_fill(user_ex_cfg, receieve_data_flag, sizeof(receieve_data_flag), CFG_ITEM_PIN_CODE, write_flag);
    custom_cfg_fill(user_ex_cfg, ble_addr, ble_addr_len, CFG_ITEM_BLE_ADDR, write_flag);
    custom_cfg_fill(user_ex_cfg, adv_data, sizeof(user_ex_cfg->adv_data_cfg.data), CFG_ITEM_ADV_IND, write_flag);
    custom_cfg_fill(user_ex_cfg, scan_rsp_data, sizeof(user_ex_cfg->scan_rsp_cfg.data), CFG_ITEM_SCAN_RSP, write_flag);
    custom_cfg_fill(user_ex_cfg, (u8 *)gap_device_name, sizeof(user_ex_cfg->ble_name_cfg.data), CFG_ITEM_BLE_NAME, write_flag);
    custom_cfg_fill(user_ex_cfg, (u8 *)profile_data, sizeof(user_ex_cfg->gatt_profile_cfg.data), CFG_ITEM_GATT_PROFILE, write_flag);
    custom_cfg_fill(user_ex_cfg, last_device_connect_linkkey, \
                    sizeof(user_ex_cfg->last_device_connect_linkkey_cfg.data), \
                    CFG_ITEM_LAST_DEVICE_LINK_KEY_INFO, write_flag);
#endif
    return 0;
}
#else
...
static u32 ex_cfg_fill_content(ex_cfg_t *user_ex_cfg, u8 *write_flag)
{
    ...
    // CFG_ITEM_EDR_ADDR
    extern u8 cur_edr_address[];
    // CFG_ITEM_PIN_CODE
    u8 receieve_data_flag[4] = {BT_CONNECTION_VERIFY, 0, 0, 0};
    // CFG_ITEM_ADV_IND
    extern u8 adv_data[];
    // CFG_ITEM_SCAN_RSP
    extern u8 scan_rsp_data[];
    // CFG_ITEM_BLE_NAME
    extern char gap_device_name[];
    // CFG_ITEM_GATT_PROFILE
    extern const uint8_t profile_data[];
    // CFG_ITEM_LAST_DEVICE_LINK_KEY_INFO
    extern u8 last_device_connect_linkkey[];

    // CFG_ITEM_BLE_ADDR
    extern u8 *get_ble_addr(u32 * ble_len);
    u32 ble_addr_len = 0;
    u8 *ble_addr = get_ble_addr(&ble_addr_len);
    u8 i = 0;
    for (; i < (ble_addr_len / 2); i++) {
        ble_addr[i] ^= ble_addr[ble_addr_len - i - 1];
        ble_addr[ble_addr_len - i - 1] ^= ble_addr[i];
        ble_addr[i] ^= ble_addr[ble_addr_len - i - 1];
    }

    custom_cfg_file_fill(user_ex_cfg, write_flag);
    custom_cfg_fill(user_ex_cfg, (u8 *)host_name, sizeof(user_ex_cfg->bt_name_cfg.data), CFG_ITEM_BT_NAME, write_flag);
    custom_cfg_fill(user_ex_cfg, cur_edr_address, sizeof(user_ex_cfg->edr_addr_cfg.data), CFG_ITEM_EDR_ADDR, write_flag);
    custom_cfg_fill(user_ex_cfg, receieve_data_flag, sizeof(receieve_data_flag), CFG_ITEM_PIN_CODE, write_flag);
    custom_cfg_fill(user_ex_cfg, ble_addr, ble_addr_len, CFG_ITEM_BLE_ADDR, write_flag);
...
u32 ex_cfg_fill_content_api(void)
{
    ...
    ex_cfg_fill_content(&user_ex_cfg, &write_flag);

#if PRINT_EX_CFG_INFO
    print_user_ex_cfg_info(&user_ex_cfg);
#endif

    if (write_flag) {
        sfc_erase(SECTOR_ERASER, exif_info.addr);
        ex_cfg_write_to_flash(&user_ex_cfg);
    }
    return 0;
}
#else
