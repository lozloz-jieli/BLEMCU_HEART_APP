...
u16 rcsp_f_read(void *fp, u8 *buff, u16 len)
{
    ...
    if((tws_ota_control(OTA_TYPE_GET) == OTA_TWS) && !get_tws_sibling_connect_state()) { //假如TWS一起升级，TWS断开了,返回失败
...
__RETRY:
    ...
    while (!((0 == __this->state) && (__this->read_len == __this->read_len))) {
        ...
        if (!((0 == __this->state) && (__this->read_len == __this->read_len))) {
